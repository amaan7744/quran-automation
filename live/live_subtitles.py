...
"""
subtitle_builder.py
Premium-style ASS subtitles for the Quran livestream: a single visually centered
subtitle block (Arabic + translation), adaptive font sizing, safe margins
for YouTube livestream, and a strict two-line cap for both
...
Arabic and its translation are treated as one centered subtitle block:
  ...
  - SAFE_TOP_MARGIN / SAFE_BOTTOM_MARGIN keep the whole block clear of
    ...
    buttons/captions at the bottom) on the livestream.
...
# Live-stream-safe font selection: no repository font files are required.
# GitHub-hosted Ubuntu runners include DejaVu Sans, and libass can resolve it.
ARABIC_FONT = os.getenv("QURAN_ARABIC_FONT", "DejaVu Sans")
ENGLISH_FONT = os.getenv("QURAN_ENGLISH_FONT", "DejaVu Sans")
...
# confirm precisely.
ARABIC_MIN_FONT_SIZE = 54   # floor for legibility after social-platform re-encoding
ARABIC_MAX_FONT_SIZE = 92  # ceiling — the size almost all chunks now use
...
# above it.
EN_MIN_FONT_SIZE = 30  # floor — never too small to read on a phone
EN_MAX_FONT_SIZE = 44  # ceiling for short translation chunks
...
BLOCK_CENTER_Y = int(PLAY_RES_Y * 0.60)
...
ARABIC_LINE_HEIGHT_FACTOR = 1.18
EN_LINE_HEIGHT_FACTOR = 1.18
# Extra breathing room between wrapped lines within the same block.
INTRA_BLOCK_LINE_GAP = 4
...
Style: Arabic,{ar_font},88,&H00FFFFFF,&H00FFFFFF,&H00101010,&H00000000,0,0,0,0,100,100,0,0,1,2.2,1.2,8,180,180,80,1
Style: English,{en_font},40,&H00F2F2F2,&H00F2F2F2,&H00101010,&H00000000,0,0,0,0,100,100,0,0,1,1.6,1.0,8,180,180,80,1
...
def build_subtitles(
    ...
    lines each so nothing ever collides or runs off the 1920x1080 canvas,
    and kept clear of the livestream safe zones throughout.
    """
    ...
    for (surah, ayah), duration in zip(batch, audio_durations):
        ...
        if ar_plan:
            ...
            for c in ar_plan:
                ...
                override = f"{{{pos}\\fad(180,120)\\fs{ar_size}}}"
        ...
        if en_plan:
            ...
            wrap_width = max(int(1500 / max(en_size, 1)), 24)
            for c in en_plan:
                en_wrapped = wrap_to_two_lines(escape_ass(c["text"]), wrap_width)
                override = f"{{{pos}\\fad(180,120)\\fs{en_size}}}"
                start = cursor + c["start"]
                end = cursor + c["end"]
                events.append(
...
# ---------------------------------------------------------------------------
# The live encoder can call build_subtitles() with the same signature used by
# the Shorts subtitle builder:
#
#     build_subtitles(batch, arabic_data, english_data, audio_durations, out)
#
# `out` is an ASS file. FFmpeg should use:
#
#     -vf "ass=/path/to/out.ass"
#
# The module intentionally does NOT import the repository's root config.py and
# does NOT require Noto Naskh Arabic or any font file committed to GitHub.
# GitHub Actions Ubuntu runners resolve DejaVu Sans through fontconfig.
#
# IMPORTANT: this module is calibrated for the livestream's 1920x1080 canvas.
# Do not reuse the old 1080x1920 constants in the live encoder.
