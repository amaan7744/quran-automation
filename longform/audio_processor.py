...
def _measure_loudness(input_args: list, pre_graph: str, target_lufs: float, target_tp: float):
    """
    ...
    `input_args` is the full `-i ...` argument list (one `-i` per source
    file) so this works identically whether `pre_graph` reads from a
    single `[0:a]` or from a multi-input concat node — see master_audio().
    """
    filter_complex = (
        f"{pre_graph};"
...
def master_audio(src, dst: Path, total_duration: float, *,
                  bitrate: str = "256k", sample_rate: int = 48000) -> None:
    """
    Applies (in order): [optional gapless concatenation of multiple
    source files —see below], high-pass rumble removal, subtle warmth/
    presence/air EQ, gentle soft-knee compression, accurate two-pass
    loudness normalization to a consistent target LUFS, a true-peak
    safety limiter, and perceptually-smooth fade in/out. Duration is
    otherwise untouched, so word/ayah timing used by the subtitle track
    remains valid.

    `src` is normally a single Path (the original Shorts/Reels call
    shape, unchanged). It may also be a list/tuple of Paths — in that
    case every source is decoded and joined via ffmpeg's sample-accurate
    `concat` FILTER (not the stream-copy concat demuxer) directly inside
    this same filter graph, in the SAME ffmpeg process that does the
    mastering. This is what surah_audio.py (long-form pipeline) uses
    instead of audio_downloader.concat_audio's "-c copy" demuxer
    concatenation: stream-copy-concatenating independently-encoded MP3
    files can leave a small gap/discontinuity at each file boundary from
    encoder delay/bit-reservoir handling — negligible over the Shorts
    pipeline's ~7-ayah batches, but compounding into audible clicks and
    real cumulative drift against the per-ayah subtitle timeline over a
    multi-hundred-ayah Surah. Decoding every source and joining PCM
    sample-for-sample removes that failure mode entirely, and folding it
    into this same pass (rather than a separate concat step before
    mastering) also means one fewer full encode/decode cycle overall,
    not one more.

    `bitrate`/`sample_rate` default to the original Shorts/Reels values;
    the long-form pipeline passes 320k/48000 for a higher-quality YouTube
    master (spec section 5) without changing Reels output.
    """
    ...
    if len(sources) > 1:
        # Normalize every input to one canonical PCM format before the
        # concat node — required for `concat` to be valid on inputs that
        # may not all share identical sample rate/channel layout, and
        # cheap relative to the mastering pass that follows.
        normalize = "".join(
            f"[{i}:a]aformat=sample_fmts=fltp:sample_rates={sample_rate}:channel_layouts=stereo[c{i}];"
            for i in range(len(sources))
        )
        joins = "".join(f"[c{i}]" for i in range(len(sources)))
        concat_prefix = f"{normalize}{joins}concat=n={len(sources)}:v=0:a=1[concatsrc];"
        in_label = "[concatsrc]"
    else:
        concat_prefix = ""
        in_label = "[0:a]"
    ...
    pre_graph = concat_prefix + _pre_dynamics_graph(in_label=in_label, out_label="shaped")

    stats = _measure_loudness(input_args, pre_graph, AUDIO_TARGET_LUFS, AUDIO_TRUE_PEAK)
    ...
    cmd = [
        ...
        # a voice-only source.
        "-c:a", "aac", "-b:a", bitrate, "-aac_coder", "twoloop", "-ar", str(sample_rate),
        str(dst),
    ]
    result = subprocess.run(cmd, capture_output=True, text=True)
    ...
    log.info(
        "Audio mastered -> %s (%d source(s), target %.1f LUFS, %s loudnorm)",
        dst.name, len(sources), AUDIO_TARGET_LUFS, "2-pass" if stats else "1-pass fallback",
    )
