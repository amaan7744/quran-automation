...
script:
  1. Reads longform.yml (via longform_config.py). If `enabled: false`,
     exits 0 immediately without touching anything.
...
NOTE ON config MODULE: this file imports from `longform_config` (NOT the
bare `config` module). `longform_config.py` is the long-form-only settings
module that lives alongside this file in longform/ — kept under a distinct
name specifically so it can never collide with the SHARED root-level
config.py that the Shorts/Reels pipeline (upload.py, performance_metadata.py,
etc.) depends on. Do not rename this import back to `config` — Python
always puts this script's own directory (longform/) first on sys.path, so
a same-named `config.py` sitting in longform/ would silently shadow the
real root config.py for every module in the process, including ones from
the Shorts pipeline that happen to get imported transitively.
"""
    ...
    if not args.force and not args.skip_schedule_check:
        ...
        if not ok:
            log.error("longform.yml schedule config is invalid: %s. Fix longform.yml, or use --force to "
                       "proceed with longform_config.py's built-in defaults.", reason)
            return 1
